...
const report = (error: unknown): { message: string; stack?: string } => {
  ...
  const formattedError = `${BOLD}[MONAASSIST ERROR] ${errorMessage}${RESET}`;
...
const warn = (message: string, error?: unknown): void => {
  console.warn(
    `${BOLD}[MONAASSIST WARN] ${message}${error ? `\n${parseErrorMessage(error)}` : ""}${RESET}`
  );
};
...
const warnDeprecated = (
  ...
  console.warn(
    `${BOLD}[MONAASSIST DEPRECATED] "${oldFeature}" is deprecated${location ? ` in ${location}` : ""}. ` +
