...
import { store } from "../../react-hooks/store";
import NewUi from "../window/main/ui";
...
const MainComponent = React.memo((props: any) => {
  ...
  const { terminal } = useAppSelector((state) => state.main.loading);
    ...
    async (selected_file: TSelectedFile) => {
      try {
        ...
        if (!pyrightProviderRef.current) {
          const provider = new MonacoPyrightProvider("./python/worker.js", {});
        ...
        if (!model) {
          ...
          const path = normalizePath(selected_file.path);
          const isTouched = store
            .getState()
            .main.active_files.find(
              (f: TActiveFile) => normalizePath(f.path) === path
            )?.is_touched;

          if (!isTouched && model.getValue() !== selected_file.content) {
  ...
  const handle_save_file = React.useCallback(
    (data: { path: string; content: string }) => {
      if (!pyrightProviderRef.current) {
        console.warn("Pyright not ready");
        return;
      }
