...
export function NewProject({ set_data }: any) {
  ...
  const [selectedPythonVersionTag, setSelectedPythonVersionTag] = useState("");
  ...
  useEffect(() => {
    const data = {
      ...
      python_version_tag: selectedPythonVersionTag && selectedPythonVersionTag,
      python_version: selectedPythonVersion && selectedPythonVersion,
      git: gitRepo,
    ...
    window.electron.ipcRenderer
      ...
      .then((versions) => {
        ...
        if (versions.length > 0) {
          ...
          setSelectedPythonVersionTag(versions[0].version_tag);
          setSelectedPythonVersion(versions[0].version);
        }
      });
  }, []);
