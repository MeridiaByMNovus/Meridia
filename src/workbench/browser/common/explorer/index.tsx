...
import useTree from "../../../services/use_file_tree";
import { useAppDispatch, useAppSelector } from "../../../react-hooks/hooks";
import { MainContext, path_join } from "../../../react-hooks/functions";
import {
  update_active_file,
  update_active_files,
  update_simple_tab,
} from "../../../react-hooks/state_manager";
import {
  IFolderStructure,
  TActiveFile,
  TFolderTree,
...
export function Explorer() {
  ...
  const { editor } = useAppSelector((s) => s.main.loading);
  ...
  const openFile = useCallback(
    async (branch_name: string, full_path: string) => {
      ...
      const file_name = branch_name;
      const normalizedPath = full_path.replace(/\\/g, "/");
      const parent_folder = normalizedPath.split("/").slice(-2, -1)[0] || "";

      const name_conflict = active_files.some(
        (f: TActiveFile) => f.name === file_name && f.path !== full_path
      );

      const display_name = name_conflict
      ...
      if (!active_files.some((f: TActiveFile) => f.path === full_path)) {
