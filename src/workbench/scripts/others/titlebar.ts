...
export async function titlebar({
  ...
  target_window: "main" | "welcomeWizard";
}) {
  ...
  if (minimizeBtn) {
    minimizeBtn.addEventListener("click", () => {
      window.electron.ipcRenderer.invoke("minimize", target_window);
    });
  }
  ...
  if (maximizeBtn) {
    maximizeBtn.addEventListener("click", async () => {
      ...
      if (isMaximized) {
        window.electron.ipcRenderer.invoke("restore", target_window);
        toggleMaxRestoreIcon(false);
      } else {
        window.electron.ipcRenderer.invoke("maximize", target_window);
  ...
  if (closeBtn) {
    closeBtn.addEventListener("click", () => {
      window.electron.ipcRenderer.invoke("close", target_window);
    });
  }
